...
class SubSampler(FullSampler):
    """ pixels are selected in an uniformly spaced grid 均匀分布的空格网格
    """
...
class NghSampler2(nn.Module):
    ...
    def gen_grid(self, step, aflow):
        ...
        if step > 0:
            ...
            shape = (B, n)
        return b1, y1, x1, shape  # b1:[4000] x1:[4000] y1:[4000]
    ...
    def forward(self, feats, confs, aflow, **kw):
        ...
        # positions in the first image 下面的self.sub_q=-8
        b1, y1, x1, shape = self.gen_grid(self.sub_q, aflow)  # b1,y1,x1: size为 [4000]的张量

        # sample features from first image
        feat1 = feat1[b1, :, y1, x1]  # size: 4000*128
        qconf = conf1[b1, :, y1, x1].view(shape) if confs else None  # size: 10*400, conf1:10*1*H*W

        # sample GT from second image
        b2 = b1
        xy2 = (aflow[b1, :, y1, x1] + 0.5).long().t()  # xy2的size为2*4000
        mask = (0 <= xy2[0]) * (0 <= xy2[1]) * (xy2[0] < W) * (xy2[1] < H)
        mask = mask.view(shape)  # mask与qconf的size一致，10*400
        ...
        xy2p = clamp(xy2[:, None, :] + self.pos_offsets[:, :, None])# self.pos_offsetsSize : 2*29 xy2p size: 2*29*4000,这种中间写None的方式可以直接将2维向量扩充为1维吗？
        pscores = (feat1[None, :, :] * feat2[b2, :, xy2p[1], xy2p[0]]).sum(dim=-1).t() # pscores size 4000*29[None,4000,128]*[None,128,29,2]None直接被忽略掉，得到4000，*128*29*2，最后一个维度的2的进行了sum累加
