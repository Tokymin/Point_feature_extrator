...
def parser() -> argparse.ArgumentParser:
    ...
    algo_opts.add_argument(
        "-r", "--pose_relation", default="trans_part",  # 默认是trans_part，但其实计算了平移和旋转的误差
    ...
    algo_opts.add_argument("-a", "--align", default=True,
    ...
    # output_opts.add_argument("--metric_csv", default=metric_csv, action="store_true",help="show plot window")
    output_opts.add_argument(
        "--plot_mode",
    ...
    usability_opts.add_argument("-v", "--verbose", default=True, action="store_true",
                                help="verbose output")
    ...
    kitti_parser.add_argument("--ref_file", default=ref_file,
                              help="reference pose file (ground truth)")
    kitti_parser.add_argument("--est_file", default=est_file, help="estimated pose file")
...
def write_metric_to_csv(metric_csv_path, data):
    ...
    csv_file.close()
    print("write metric to csv file end")


def write_align_data_to_csv(save_path, data):
    csv_file = open(save_path, "a+", encoding='utf-8', newline='')  # a+追加写入
    csv_writer = csv.writer(csv_file)
    # metrc_data_list = [test_dataset_index]
    for item in data:
        # metrc_data_list.append(item)
    ...
    csv_file.close()
    print("write align_data to csv file end")
...
def rpe(traj_ref: PosePath3D, traj_est: PosePath3D,
    ...
    print(f'correct_scale::{correct_scale},align::{align}')
    ...
    rpe_result.stats.update({"rpe_rotation_mean": np.mean(rpe_metric.rotation_error)})
    rpe_result.stats.update({"rpe_rotation_std": np.std(rpe_metric.rotation_error)})

    ape_metric = metrics.APE(pose_relation)
    ape_metric.process_data(data)

    ape_result = ape_metric.get_result(ref_name, est_name)
    ape_result.stats.update({"ape_rotation_mean": np.mean(ape_metric.rotation_error)})
...
def run(args: argparse.Namespace) -> None:
    ...
    save_results = False  # 用来保存对齐后的位姿，ICIRA时用的，不用时记得注释掉
    if save_results:
        ref_ = result.trajectories[ref_name]
        est_ = result.trajectories[est_name]  # 对齐后的
        datas = np.concatenate([est_.positions_xyz, est_.orientations_quat_wxyz], 1)
        print(f'{est_}')
        # datas = np.concatenate([np.reshape(est_.timestamps, (151, 1)), datas], 1)
        write_align_data_to_csv(modified_csv_path, datas)
        # logger.debug(SEP)
