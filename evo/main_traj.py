...
def write_aligin_data_to_csv(write_quat_file_path, trans_data, quat_data_wxyz):
    csv_file = open(write_quat_file_path, 'w', encoding='utf-8', newline='')
    csv_writer = csv.writer(csv_file)
    time = 0
    for i in range(len(trans_data)):
        csv_writer.writerow(
            [trans_data[i, 0], trans_data[i, 1], trans_data[i, 2], quat_data_wxyz[i, 1], quat_data_wxyz[i, 2],
             quat_data_wxyz[i, 3], quat_data_wxyz[i, 0],
             time])  # tx,ty,tz,rx,ry,rz,rw,time
        time = time + 1.000002
    csv_file.close()
    print("write aligin data to csv file end")
...
def run(args):
    ...
    synced_refs = {}
    if synced:
        from evo.core import sync  # 同步和对齐
        if not args.ref:
            logger.debug(SEP)
    ...
    if args.plot or args.save_plot or args.serialize_plot:
        ...
        if args.ref:
            ...
            logger.info("plot_422::")
            # 这个traj是refence的，不知道这个ax_traj是什么，可能就是对齐后的轨迹(猜测错误)
            # print(ax_traj)
        ...
        for name, traj in trajectories.items():
            ...
            # 这里就是画数据的轨迹了
            # print(type(traj))
            # print(traj.get_statistics())
            # print(traj._positions_xyz)
            # print(traj.orientations_quat_wxyz)
            # 写入一个csv文件
            write_file_path = r"E:\Toky\sc_depth_pl_modified\evo\output\output.csv"
            write_aligin_data_to_csv(write_file_path, traj._positions_xyz, traj.orientations_quat_wxyz)
